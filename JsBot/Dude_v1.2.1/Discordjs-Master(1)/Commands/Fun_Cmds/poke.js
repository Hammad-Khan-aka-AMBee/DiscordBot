...
module.exports= {
	name: 'poke',
	description : ' fun command of poking ',
	execute(message, args) {
		async function Display()  {
	if (message.content.startsWith('!poke')){	
		var user = message.mentions.users.first()// || client.users.get(args[0]);
			if (!user) return message.channel.send( 'You need to mention user you want to poke')//, client.commands.get('hug').help);
			
			const  body  = await fetch('http://api.nekos.fun:8080/api/poke').then(response => response.json());
			...
			embed.setDescription(`**${message.author.username}** poked **${user.username}**`)
