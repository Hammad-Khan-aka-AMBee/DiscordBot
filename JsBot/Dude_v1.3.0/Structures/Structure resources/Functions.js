...
let GScore = client.db.prepare("SELECT * FROM Members WHERE UserID = ? AND GuildID = ?");
let SScore = client.db.prepare("INSERT OR REPLACE INTO Members (_ID, UserID, GuildID, Xp, Lvl) VALUES (@_ID, @UserID, @GuildID, @Xp, @Lvl)");
...
/*
    setXp : (message) => {
        ...
        const score =
            ...
            GScore.get(message.author.id, message.guild.id)
