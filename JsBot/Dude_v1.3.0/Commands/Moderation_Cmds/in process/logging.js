...
`)
              ...
              if (option.match("set")) {
            var channel = JSON.parse(fs.readFileSync("../../Database/database/logging.json", "utf8"))
            if (!message.member.hasPermission("MANAGE_GUILD") && message.author.id !== '637598340154130432') return message.reply(`**${message.author.username}**, Sorry, You need permission \`Manage Guild\` to use this command!`);
            var inputmessage = message.mentions.channels.first()
            if (args[0]) {
              channel[message.guild.id] = {
                channel: inputmessage.id
             };
              fs.writeFile("../../Datatbase/database/logging.json", JSON.stringify(channel, null, 2), (err) => {
            ...
            if (option.match("on")) {
            var welcomesetting = JSON.parse(fs.readFileSync("../../Database/database/logonoff.json", "utf8"));
            welcomesetting[message.guild.id] = {
                checker: 1
                };
                  fs.writeFile("../../Database/database/logonoff.json", JSON.stringify(welcomesetting, null, 2), (err) => {
                    console.error(err)
                 })
