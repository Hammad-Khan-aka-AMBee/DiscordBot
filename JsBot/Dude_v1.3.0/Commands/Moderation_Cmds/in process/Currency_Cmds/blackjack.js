...
const { shuffle, verifyText } = require('../../Structures/Util');
...
module.exports= {
	...
	if (message.content.startsWith('!blackjack')){		
		...
		let amount = parseInt(args[0]);
			...
			if (!amount) return message.channel.send( 'You need to specify amount to bet. blackjack <bet>' )//, client.commands.get('blackjack').help)
			if (isNaN(amount)) return message.channel.send(`**${message.author.username}**, Please enter valid number!`);
			if (amount > 9999) return message.channel.send('Bet max to 999 only');
			...
			let curBal = bal[message.author.id].balance;
				...
				try {
					...
					while (!win) {
						if (playerTurn) {
							let m =  message.channel.send(`**First Dealer Card:** ${dealerHand[0].display}\n**You (${calculate(playerHand)}):**\n${playerHand.map(card => card.display).join('\n')}\n\n_Hit?_ *(yes/no)*`);
							const hit = verifyText(message.channel, message.author);
							m.delete();
							if (hit) {
								...
								message.channel.send(`Second dealer card is ${dealerHand[1].display}, total of ${dealerTotal}.`);
								playerTurn = false;
							}
						} else {
							...
							if (total > 21) {
								...
								message.channel.send(`Dealer drew ${card.display}, total of ${total}.`);
							}
						}
					}
					decks.delete(message.channel.id);
